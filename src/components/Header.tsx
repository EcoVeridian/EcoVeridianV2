...
export default function Header() {
  ...
  const navigationItems = settings.nav
    .filter((item) => item.visible)
    .map((item) => ({ to: item.route, label: item.label }));
          ...
          <nav className="hidden md:flex items-center gap-6 lg:gap-8 ml-4">
            {navigationItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `text-body-md font-sans pb-1 cursor-pointer transition-all duration-200 text-sm tracking-wide ${
                    isActive
                      ? 'text-primary font-bold border-b-2 border-primary'
                      : 'text-on-surface-variant hover:text-primary'
                  }`
                }
                id={`nav-${item.to === '/' ? 'home' : item.to.slice(1)}`}
              >
                {item.label}
              </NavLink>
            ))}
          ...
          <nav className="flex flex-col gap-3 px-6 pb-2">
            {navigationItems.map((item) => (
              <NavLink
                ...
                onClick={() => setMobileMenuOpen(false)}
                className={({ isActive }) =>
                  `text-left text-body-md font-sans py-2 border-l-2 pl-3 transition-colors ${
                    isActive
                      ? 'border-primary text-primary font-bold bg-surface-container-low'
                      : 'border-transparent text-on-surface-variant'
                  }`
                }
                id={`mobile-nav-${item.to === '/' ? 'home' : item.to.slice(1)}`}
              >
                {item.label}
              </NavLink>
            ))}
