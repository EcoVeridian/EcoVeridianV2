...
import { Loader2, AlertTriangle, Plus, ArrowUp, ArrowDown, Pencil, Trash2, FileText } from 'lucide-react';
...
export default function ArticlesList() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<ArticleDoc | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      const docs = snap.docs.map((d) => d.data() as ArticleDoc);
      docs.sort((a, b) => a.order - b.order);
      setArticles(docs);
      setState({ phase: 'ready' });
    } catch (err) {
      setState({ phase: 'error', message: err instanceof Error ? err.message : 'Failed to load articles' });
    }
  };
  ...
  const swap = async (index: number, otherIndex: number) => {
    if (otherIndex < 0 || otherIndex >= articles.length) return;
    const a = articles[index];
    const b = articles[otherIndex];
    setBusySlug(a.slug);
    try {
      const db = adminDb();
      await Promise.all([
        setDoc(doc(db, 'articles', a.slug), { order: b.order, updatedAt: serverTimestamp() }, { merge: true }),
        setDoc(doc(db, 'articles', b.slug), { order: a.order, updatedAt: serverTimestamp() }, { merge: true }),
      ]);
      const next = [...articles];
  ...
  const handleDelete = async () => {
    ...
    try {
      const db = adminDb();
      await deleteDoc(doc(db, 'articles', deleteTarget.slug));
  ...
  return (
    <div className="animate-fade-in">
      <header className="mb-8 border-b-[0.5px] border-outline-variant pb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="font-serif text-3xl md:text-4xl font-bold text-primary flex items-center gap-3">
            <FileText className="w-8 h-8 text-secondary stroke-[1.5]" />
            Articles
          </h1>
          <p className="font-sans text-sm text-on-surface-variant mt-2 leading-relaxed max-w-xl">
            Research articles shown on the home page and /articles. Drafts are hidden from the public
      ...
      {state.phase === 'ready' && articles.length === 0 && (
        <p className="font-sans text-sm text-on-surface-variant py-8">
          No articles yet — create one or run Import Defaults.
        </p>
      )}
            ...
            >
              <div className="min-w-0 flex-grow">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <h2 className="font-serif font-bold text-primary truncate">{article.title || '(untitled)'}</h2>
                ...
                <p className="font-sans text-xs text-on-surface-variant">{article.category}</p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  onClick={() => swap(index, index + 1)}
                  disabled={index === articles.length - 1 || busySlug !== null}
                  className="p-1.5 border-[0.5px] border-outline rounded-[2px] hover:border-primary transition-colors disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
