...
import { PageTeamDoc } from '../../../types';
import { SEED_PAGE_TEAM } from '../../../content/seeds';
...
export default function TeamPageEdit() {
  const [loaded, setLoaded] = useState<PageTeamDoc | null>(null);
  const [form, setForm] = useState<PageTeamDoc>(SEED_PAGE_TEAM);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const db = adminDb();
        const snap = await getDoc(doc(db, 'pages', 'team'));
        if (cancelled) return;
        const data = snap.exists() ? (snap.data() as PageTeamDoc) : SEED_PAGE_TEAM;
        setLoaded(data);
        setForm(data);
  ...
  const update = <K extends keyof PageTeamDoc>(key: K, value: PageTeamDoc[K]) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const db = adminDb();
      await setDoc(doc(db, 'pages', 'team'), { ...form, updatedAt: serverTimestamp() }, { merge: true });
      setLoaded(form);
  ...
  return (
    <div className="animate-fade-in">
      <header className="mb-8 border-b-[0.5px] border-outline-variant pb-6">
        ...
        <h1 className="font-serif text-3xl md:text-4xl font-bold text-primary">Team</h1>
      </header>
      ...
      <div className="flex flex-col gap-6 max-w-3xl">
        ...
        <TextArea label="Intro" value={form.intro} onChange={(v) => update('intro', v)} rows={3} />

        <Field label="Interns Kicker" value={form.internsKicker} onChange={(v) => update('internsKicker', v)} />
        <Field label="Interns Heading" value={form.internsHeading} onChange={(v) => update('internsHeading', v)} />
        <TextArea
          label="Interns Intro"
          value={form.internsIntro}
          onChange={(v) => update('internsIntro', v)}
          rows={3}
        />
      </div>
